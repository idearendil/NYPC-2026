<p align="right">
  <a href="README.md"><img alt="English" src="https://img.shields.io/badge/README-English-6e7681?style=for-the-badge"></a>
  <a href="README.ko.md"><img alt="한국어" src="https://img.shields.io/badge/README-%ED%95%9C%EA%B5%AD%EC%96%B4-1f6feb?style=for-the-badge"></a>
</p>
...
| 구성 | 파일 | 설명 |
|---|---|---|
| 환경 | `fast_env.py` | 심판의 게임 규칙을 배치 텐서 연산으로 재작성. `B`개의 게임을 GPU에서 동시에 진행하며 `testing-tool2.py`와 비트 단위로 일치. |
| 학습기 | `ppo_selfplay.py` | 그 환경 위의 self-play PPO. 2단계로 분해된 actor, 특권 정보를 받는 critic, 상대 풀, 멀티 GPU. |
| 제출 봇 | `vanilla_bot.py` | 학습된 actor를 순수 numpy로 재구현. 턴당 추론 1회, 탐색 없음. `data.bin`을 읽음. |
...
torch를 **먼저** 각자 CUDA 빌드에 맞춰 설치한 뒤 나머지를 설치합니다.
...
# 5. 실제로 두어 보고, 세기를 측정
python run_match.py --seed 42                     # replay.log 생성
...
`final_rush_bot.py`, `final_rush_bot2.py`, `final_defence_bot.py`를 배치·벡터화해
이식한 것 — 이고 절대 교체되지 않습니다. 덕분에 풀이 자가 대전 동질화로 무너지지
않습니다. 에이전트가 모든 상대를 `pool_add_threshold` 이상으로 이기면 자기 자신을
풀에 스냅샷하고, `perm_snapshot_every` iteration마다 그 스냅샷이 영구 슬롯이 됩니다.
...
```bash
python test_fast_env.py    # 턴 종료 시점 심판과의 비트 단위 일치 (느림)
python test_phases.py      # 페이즈마다 비트 단위 일치, cpu + cuda
python test_observe.py     # 관측 shape, 크기가 섞인 배치의 토큰 마스크
python test_encoder.py     # 모든 인코더 피처를 독립적으로 재계산해 대조
python verify_np_bot.py    # numpy 봇 == torch 파이프라인 (checkpoint.pt + data.bin 필요)
```
...
- `python ppo_selfplay.py --smoke`가 가장 빠른 전 구간 점검입니다. 작은 네트워크,
