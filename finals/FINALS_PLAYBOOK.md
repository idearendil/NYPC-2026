...
5. **네트워크** — 폭(`d_model=64` 기본), 층수(MLP 2층 / 트랜스포머 2~3블록),
   행동 헤드 구성. **작게 시작한다.**
6. **휴리스틱 봇** — 가장 단순한 공격형 하나. 5단계에서 상대 풀에 넣는다.
...
phase_iters: 100
phases:
  - {lr: 0.001,   epochs: 4, ent_coef: 0.02}
  - {lr: 0.0005,  epochs: 3, ent_coef: 0.005}
  - {lr: 0.0002,  epochs: 3, ent_coef: 0.001}
...
4. 관측 피처 → 승패에 직결되는 것만
5. 페이즈 스케줄 → `phases: null` + 고정 lr
6. 정합성 검증 → 200턴 × 8게임 → 20턴 × 4게임 (**0은 안 된다**)
7. 그래도 안 되면 **`B`를 줄이고** 학습 시간을 확보한다
