...
HQ_LEVELS = [
    ...
    HqLevel(1000,  6, 20, 2, 2, 3),
    HqLevel(2000,  7, 25, 3, 2, 4),
    HqLevel(3000,  8, 30, 3, 3, 5),
]
BASE_LEVELS = [
    BaseLevel(0,    0,  0, 0),
    BaseLevel(500,  6, 1, 1),
    BaseLevel(550, 12, 1, 2),
    BaseLevel(600, 18, 2, 3),
]
...
def generate_map(rng: XoShiro256, NP: Optional[int] = None,
    ...
    if NP is not None:
        ...
        if N < 181 or N > 249:
            raise ValueError(
                f"N = 2*NP+1 = {N} is out of range [181, 249] (got NP={NP})"
            )
    else:
        NP = rng.next_u64_incl(90, 124)

    N = 2 * NP + 1

    root_n = math.sqrt(N)
    K_lo_raw = math.ceil(root_n - 1.0)
    K_hi_raw = math.floor(root_n + 4.0)
    K_lo = K_lo_raw + 1 if K_lo_raw % 2 == 0 else K_lo_raw
    K_hi = K_hi_raw - 1 if K_hi_raw % 2 == 0 else K_hi_raw
    KP_lo = (K_lo - 1) // 2
    KP_hi = (K_hi - 1) // 2
    ...
    if KP is not None:
        ...
        if K < K_lo or K > K_hi:
            raise ValueError(
                ...
                f"{K_lo} <= K <= {K_hi} (odd), so {KP_lo} <= KP <= {KP_hi} "
                f"(got KP={KP})"
            )
    else:
...
def apply_upgrades(st: GameState, m: MapData, side: Side,
    ...
    for r in sub.upgrades:
        ...
        if r not in st.buildings:
            ...
            if st.gold[u] < BASE_LEVELS[1].cost:
                raise WaError(side, f"UPGRADE (build) insufficient gold")
            st.gold[u] -= BASE_LEVELS[1].cost
...
def _hop_set(start: int, H: int, adj: List[List[int]]) -> set:
    seen = {start}
    frontier = [start]
    for _ in range(H):
        nxt = []
...
def send_result_block(player: Player, side: Side, st: GameState, m: MapData,
    ...
    up_rows = sorted(rb_self.upgrades, key=lambda r: r[0])
    emit(player, side, "UPGRADE", len(up_rows))
    for (region, up_side) in up_rows:
        emit(player, side, up_side.letter, region)

    rec_rows = sorted(rb_self.trained_keys, key=wkey_sort)
    emit(player, side, "TRAIN", len(rec_rows))
    if rec_rows:
        emit(player, side, " ".join(id_str(k) for k in rec_rows))

    mv_rows = sorted(rb_self.moves, key=lambda r: wkey_sort(r[0]))
    emit(player, side, "MOVE", len(mv_rows))
    for (key, new_region) in mv_rows:
        emit(player, side, id_str(key), new_region)

    dmg_rows = sorted(rb_self.damages, key=lambda r: (CAUSE_ORDER[r[0]], wkey_sort(r[1])))
    emit(player, side, "DAMAGE", len(dmg_rows))
    for (cause, key, dmg) in dmg_rows:
        emit(player, side, cause, id_str(key), dmg)

    sg_rows = sorted(rb_self.sieges, key=lambda r: r[1])
    emit(player, side, "SIEGE", len(sg_rows))
    for (sg_side, region, dmg) in sg_rows:
        emit(player, side, sg_side.letter, region, dmg)

    vis = set()
    for w in st.warriors.values():
        if w.side.value == u and w.hp > 0:
            vis |= _hop_set(w.region, HOP_VISION, m.adj)
    for b in st.buildings.values():
        if b.side.value == u:
            vis |= _hop_set(b.region, HOP_VISION, m.adj)

    wrows = [w for w in st.warriors.values()
             if w.hp > 0 and (w.side.value == u or w.region in vis)]
    wrows.sort(key=lambda w: wkey_sort(w.key))
    emit(player, side, "WARRIOR", len(wrows))
    for w in wrows:
        emit(player, side, id_str(w.key), w.region, w.hp)

    brows = [b for b in st.buildings.values()
             if b.side.value == u or b.region in vis]
    brows.sort(key=lambda b: b.region)
    emit(player, side, "BUILDING", len(brows))
    for b in brows:
        kind = "HQ" if b.kind is BKind.HQ else "BASE"
        emit(player, side, b.side.letter, b.region, kind, b.level, b.hp)
...
def run_game(m: MapData, players: List[Player], log: TextIO) -> str:
    ...
    for st.day in range(1, MAX_DAYS + 1):
        ...
        send_result_block(players[0], Side.LEFT,  st, m, t_used_l, t_used_r, rb_l)
        send_result_block(players[1], Side.RIGHT, st, m, t_used_r, t_used_l, rb_r)
