...
| 구성 | 파일 | 설명 |
|---|---|---|
| 환경 | `src/fast_env.py` | 본선 게임 규칙을 배치 텐서 연산으로 재작성. `B`개의 게임을 GPU에서 동시에 진행하며 `judge/testing-tool2.py`와 비트 단위로 일치. |
...
- **안개:** 적 유닛과 건물은 내 전사나 건물로부터 **2칸 이내**에서만 보입니다. 그
  바깥은 기억하고 추측해야 합니다.
- **승리 조건:** 상대 본부를 파괴하거나, 400일이 지난 시점에 본부 체력이 앞서면
  이깁니다.
...
아래 명령들은 모두 **저장소 루트**에서 실행해야 합니다. torch를 **먼저** 각자 CUDA 빌드에
맞춰 설치한 뒤 나머지를 설치하시면 됩니다.
...
끌 수 있음). 인증 정보는 환경변수 `WANDB_API_KEY`나 `wandb login`에서 가져옵니다.

## 동작 원리

**배치 환경.** 강화학습에는 게임 하나당 프로세스 하나를 띄우는 기존 시뮬레이터가 감당할 수 없는
양의 대국이 필요합니다. 그래서 `fast_env.py`가 규칙 전체를 `B`개 게임에 대한 텐서
연산으로 재구현합니다. 시뮬레이터와 **비트 단위로 일치**함을 검증했습니다. 턴 종료 시점(`test_fast_env.py`)과
...
**분해된 행동 공간.** 한 턴의 행동은 선택 하나가 아니라 명령의 *집합*이기 때문에, actor를
둘로 나눴습니다.
...
critic 모두 7차원 보조 head가 있습니다 — 거점별로 1~5턴 안에 도달 가능한 적 병력, 그리고
상대의 숨겨진 금화에 대한 전역 추정치입니다. 보조 타깃은 인코더를 다듬기 위한 학습
전용 감독 신호이고, 제출 봇은 이 head를 돌리지 않습니다.
...
**제출 봇.** torch는 import만으로 약 2.3초라 대회 환경의 1초 제한 안에 들어가지
...
대한 예측 신뢰도(종류, 레벨, 체력, 주둔 병력, 그리고 그 관측의 나이)을 들고 다니면서, 직접
계산한 시야 안에서는 갱신하고 그 뒤에는 나이를 증가시킵니다. `fast_env`의 안개 처리를
...
- 대회 때 사용한 가중치는 저장소에 없습니다(최종 제출 가중치는 2ff2ec3 커밋에서 복원할 수 있습니다).
- 전부 GPU에서 돌아가며, `--gpus N`은 단순 데이터 병렬입니다(iteration당 데이터는
  같고 랭크로 나뉠 뿐). `minibatch, B`는 GPU 수로 나누어떨어져야 합니다.
